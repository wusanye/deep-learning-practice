...
class AlexNet(object):
    ...
    def forward(x, output_dim, keep_prob):
        ...
        with tf.variable_scope('conv1'):
            conv1 = conv_relu(x, [3, 3, 3, 64], 1)
            pool1 = max_pool(conv1, ksize=2, stride=2, padding='SAME')
            layers.append(pool1)

        with tf.variable_scope('conv2'):
            conv2 = conv_relu(layers[-1], [3, 3, 64, 96], 1)
            pool2 = max_pool(conv2, ksize=2, stride=2, padding='SAME')
            layers.append(pool2)

        with tf.variable_scope('conv3'):
            conv3 = conv_relu(layers[-1], [3, 3, 96, 192], 1)
            layers.append(conv3)

        with tf.variable_scope('conv4'):
            conv4 = conv_relu(layers[-1], [3, 3, 192, 192], 1)
            layers.append(conv4)

        with tf.variable_scope('conv5'):
            conv5 = conv_relu(layers[-1], [3, 3, 192, 96], 1)
